...
import { Users, Shield, MapPin, Settings, Plus, Edit2, Trash2, ChevronRight, UserPlus, Calendar, Search, Check, X, Loader2, MessageSquare, Crown } from 'lucide-react';
...
export default function MotoClubsHub() {
  ...
  const renderDiscover = () => (
    <div className="space-y-6">
      <h2 className="text-2xl font-display font-black uppercase italic tracking-tight text-white mb-6">Discover MotoClubs</h2>
          ...
          return (
            <div key={club.club_id} className="glass-card p-6 flex flex-col">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-16 h-16 rounded-2xl overflow-hidden border border-white/10 bg-carbon shrink-0">
                  {club.logo_url ? (
                ...
                <div>
                  <h3 className="text-xl font-display font-black uppercase italic tracking-tight text-white flex items-center gap-1.5">
                    {club.name}
              ...
              {isMember ? (
                <div className="text-center py-2 px-4 rounded-xl text-xs font-mono uppercase tracking-widest border border-white/5 bg-asphalt/50 text-steel">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-mono font-black text-[10px] uppercase tracking-widest whitespace-nowrap transition-all ${
                  activeTab === 'discover' 
                    ...
                    : 'bg-carbon/50 text-steel hover:text-white border border-white/5'
                }`}
              >
            ...
            {ambassadorStatus === 'pending' ? (
              <div className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 font-mono font-bold text-[10px] uppercase tracking-widest">
                <Shield className="w-4 h-4" />
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-6 py-3 rounded-2xl bg-white/5 hover:bg-white/10 text-white border border-white/10 transition-all font-mono font-black text-[10px] uppercase tracking-widest"
              >
                <Crown className="w-4 h-4 text-primary" />
                {isAmbassador ? 'Create MotoClub' : 'Apply to Create Club'}
              </button>
            )}
          </div>
                          ...
                          <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-xl transition-all ${
                              selectedClubId === club.club_id 
                                ...
                                : 'bg-asphalt/30 border border-white/5 text-steel hover:text-white hover:border-white/20'
                            }`}
                          >
                ...
                <div className="flex-1">
                  {selectedClubId ? (
                    <ClubManager 
                      ...
                      isOwner={ownedClubs.some(c => c.club_id === selectedClubId)} 
...
function ClubManager({ clubId, isOwner, membershipId, onLeave, onChat }: { clubId: number, isOwner: boolean, membershipId?: number, onLeave?: () => void, onChat: (name: string) => void }) {
  ...
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'overview' | 'members' | 'roles' | 'chapters'>('overview');

  // ... (inside ClubManager render)
  <div className="flex justify-between items-center mb-6">
    <h2 className="text-2xl font-bold text-white">{clubData?.name}</h2>
    <button onClick={() => onChat(clubData?.name)} className="btn-secondary py-2 px-4 text-xs flex items-center gap-2">
    ...
    <div className="space-y-8">
      <div className="glass-card p-6 sm:p-8">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-8">
          <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-3xl overflow-hidden border border-white/10 shadow-2xl shrink-0 bg-carbon">
            {club.logo_url && <img src={club.logo_url} alt={club.name} className="w-full h-full object-cover" />}
          </div>
          <div className="flex-1">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-3xl sm:text-4xl font-display font-black uppercase italic tracking-tight text-white flex items-center gap-2">
                {club.name}
            ...
            <div className="flex flex-wrap gap-6 text-[10px] font-mono text-steel uppercase tracking-widest">
              <span className="flex items-center gap-2 bg-asphalt/50 px-3 py-1.5 rounded-full border border-white/5">
                <Users className="w-4 h-4 text-primary" /> {approvedMembers.length} {t('club.mgmt.members')}
              </span>
              <span className="flex items-center gap-2 bg-asphalt/50 px-3 py-1.5 rounded-full border border-white/5">
                <MapPin className="w-4 h-4 text-primary" /> {chapters.length} {t('club.mgmt.chapters')}
              </span>
            </div>
            {!isOwner && membershipId && onLeave && (
              <button 
                onClick={onLeave}
                className="mt-6 btn-secondary py-2 px-4 text-xs text-red-500 hover:text-red-400 border-red-500/20 hover:border-red-500/50"
              >
      ...
      <div className="flex overflow-x-auto gap-2 pb-2 no-scrollbar">
        {['overview', 'members', 'roles', 'chapters'].map(tab => {
          if (!isOwner && (tab === 'roles' || tab === 'chapters')) return null;
          return (
            <button
              key={tab}
              onClick={() => setActiveTab(tab as any)}
              className={`px-4 py-2 rounded-xl font-mono font-bold text-[10px] uppercase tracking-widest transition-all ${
                activeTab === tab ? 'bg-white/10 text-white' : 'text-steel hover:text-white hover:bg-white/5'
              }`}
            >
              {t(`club.mgmt.${tab}`)}
              {tab === 'members' && pendingMembers.length > 0 && isOwner && (
                <span className="ml-2 bg-primary text-asphalt px-1.5 py-0.5 rounded-full text-[8px]">{pendingMembers.length}</span>
              )}
            </button>
          );
        })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {isOwner && (
            <div className="glass-card p-6">
              <h3 className="text-xl font-display font-black uppercase italic tracking-tight text-white mb-4">{t('club.mgmt.pendingApps')}</h3>
              {pendingMembers.length === 0 ? (
                <p className="text-steel font-light text-sm">{t('club.mgmt.noPending')}</p>
              ) : (
                <div className="space-y-4">
                  {pendingMembers.map((m: any) => (
                    <div key={m.id} className="flex items-center justify-between bg-asphalt/30 p-4 rounded-xl border border-white/5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full overflow-hidden bg-carbon">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleMemberStatus(m.id, 'approved')} className="p-2 bg-primary/20 text-primary rounded-lg hover:bg-primary hover:text-asphalt transition-colors" title={t('common.approve')}><Check className="w-4 h-4" /></button>
                        <button onClick={() => handleMemberStatus(m.id, 'rejected')} className="p-2 bg-red-500/20 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-colors" title={t('common.reject')}><X className="w-4 h-4" /></button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className={`glass-card p-6 ${!isOwner ? 'md:col-span-2' : ''}`}>
            <h3 className="text-xl font-display font-black uppercase italic tracking-tight text-white mb-4">{t('club.mgmt.recentMembers')}</h3>
            <div className="space-y-4">
              {approvedMembers.slice(0, 5).map((m: any) => (
                <div key={m.id} className="flex items-center gap-3 bg-asphalt/30 p-4 rounded-xl border border-white/5">
                  <div className="w-10 h-10 rounded-full overflow-hidden bg-carbon">
                    {m.avatar_url && <img src={m.avatar_url} className="w-full h-full object-cover" />}
                  </div>
                  <div>
      ...
      {activeTab === 'members' && (
        <div className="glass-card p-6">
          <h3 className="text-xl font-display font-black uppercase italic tracking-tight text-white mb-6">{t('club.mgmt.members')}</h3>
          <div className="space-y-4">
            {approvedMembers.map((m: any) => (
              <div key={m.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-asphalt/30 p-4 rounded-xl border border-white/5">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full overflow-hidden bg-carbon">
                    {m.avatar_url && <img src={m.avatar_url} className="w-full h-full object-cover" />}
                  </div>
                ...
                {isOwner ? (
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => handleMemberStatus(m.id, 'banned')} className="text-red-500 hover:text-red-400 p-2" title={t('common.delete')}>
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                ) : (
                  <div className="text-[10px] font-mono text-steel uppercase tracking-widest bg-white/5 px-3 py-1 rounded-full border border-white/5">
                    {roles.find((r: any) => r.id === m.role_id)?.name || t('club.mgmt.noRole')}
                  </div>
                )}
              </div>
      ...
      {activeTab === 'chapters' && (
        <ChapterManager clubId={clubId} chapters={chapters} onUpdate={fetchClubData} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-display font-black uppercase italic tracking-tight text-white">{t('club.mgmt.roles')}</h3>
      ...
      {isAdding && (
        <form onSubmit={handleCreate} className="glass-card p-6 space-y-4">
          <div>
            ...
            <input type="text" required value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} className="input-field" placeholder="e.g. Road Captain" />
          </div>
          <div>
            <label className="block text-[10px] font-mono uppercase tracking-widest text-steel mb-2">{t('club.mgmt.roleDesc')}</label>
            <input type="text" value={formData.description || ''} onChange={e => setFormData({...formData, description: e.target.value})} className="input-field" placeholder="Role responsibilities" />
          </div>
        ...
        {roles.map(role => (
          <div key={role.id} className="glass-card p-6 flex flex-col">
            <div className="flex justify-between items-start mb-2">
              ...
              <button onClick={() => handleDelete(role.id)} className="text-steel hover:text-red-500 transition-colors">
                <Trash2 className="w-4 h-4" />
              </button>
            </div>
...
function ChapterManager({ clubId, chapters, onUpdate }: { clubId: number, chapters: any[], onUpdate: () => void }) {
  const [isAdding, setIsAdding] = useState(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-display font-black uppercase italic tracking-tight text-white">{t('club.mgmt.chapters')}</h3>
        <button onClick={() => setIsAdding(!isAdding)} className="btn-primary py-2 px-4 text-xs flex items-center gap-2">
      ...
      {isAdding && (
        <form onSubmit={handleCreate} className="glass-card p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-[10px] font-mono uppercase tracking-widest text-steel mb-2">{t('club.mgmt.chapterName')}</label>
              <input type="text" required value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} className="input-field" placeholder="e.g. Central Chapter" />
            </div>
            <div>
          ...
          <div>
            <label className="block text-[10px] font-mono uppercase tracking-widest text-steel mb-2">{t('club.mgmt.roleDesc')}</label>
            <input type="text" value={formData.description || ''} onChange={e => setFormData({...formData, description: e.target.value})} className="input-field" placeholder="Chapter details" />
          </div>
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setIsAdding(false)} className="btn-secondary py-2 px-4 text-xs">{t('common.cancel')}</button>
            <button type="submit" className="btn-primary py-2 px-4 text-xs">{t('club.mgmt.saveChapter')}</button>
          </div>
        </form>
      )}
        ...
        {chapters.map(chapter => (
          <div key={chapter.id} className="glass-card p-6 flex flex-col">
            <div className="flex justify-between items-start mb-4">
              ...
              <button onClick={() => handleDelete(chapter.id)} className="text-steel hover:text-red-500 transition-colors">
                <Trash2 className="w-4 h-4" />
              </button>
            </div>
